...
test('landing page loads and has key elements', async ({ page }) => {
    ...
    // Check Title
    await expect(page).toHaveTitle(/WorkMila.*recruitment platform/i);
    ...
    // Check Features section (Trending Sectors)
    await expect(page.getByText(/Software Engineering/i)).toBeVisible();

    // Verify Footer
    await expect(page.getByText(/WorkMila/).first()).toBeVisible();
});
