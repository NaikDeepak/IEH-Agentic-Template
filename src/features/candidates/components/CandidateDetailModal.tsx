...
import {
    X, User, Briefcase, MapPin, Star, Loader2,
    Bookmark, BookmarkCheck, Globe, Github, Linkedin,
    Clock, Target, MessageSquare, Send, Check,
} from 'lucide-react';
...
export const CandidateDetailModal: React.FC<CandidateDetailModalProps> = ({
    ...
    const handleContact = async () => {
        ...
        try {
            ...
            setContactSent(true);
            setContactMessage('');
            setTimeout(() => { setContactSent(false); setShowContact(false); }, 2500);
        } catch (err) {
                    ...
                    <div className="overflow-y-auto flex-1 px-6 py-5 space-y-5">
                        ...
                        <div className="flex items-start gap-4">
                            ...
                            {candidate.matchScore !== undefined && (
                                <span className={`shrink-0 px-2.5 py-0.5 border rounded-full text-xs font-semibold flex items-center gap-1 ${getMatchColor(candidate.matchScore)}`}>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            {candidate.experience && (
                                <FactChip icon={<Briefcase className="w-3.5 h-3.5" />} label="Experience" value={candidate.experience} />
                            )}
                            {candidate.availability && (
                                <FactChip icon={<Clock className="w-3.5 h-3.5" />} label="Availability" value={candidate.availability} />
                            )}
                            {candidate.preferredRole && (
                                <FactChip icon={<Target className="w-3.5 h-3.5" />} label="Preferred Role" value={candidate.preferredRole} />
                            )}
                        </div>
...
interface FactChipProps { icon: React.ReactNode; label: string; value: string }
