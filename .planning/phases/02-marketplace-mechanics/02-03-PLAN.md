...
<task type="auto">
  ...
  <action>
    Extend the `reaper` function (or create a separate `notifier` function if preferred for timing).

    Logic:
    1. Query items where `status == 'active'` AND `expiresAt` is between 24h and 25h from now.
    2. For each item, send an email using `nodemailer`.
    3. Use a simple HTML template: "Your [Job/Profile] will expire in 24 hours. Login now to keep it active."
...
- [ ] Warning query defined (24h window)
