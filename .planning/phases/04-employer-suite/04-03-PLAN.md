...
<task type="auto">
  ...
  <verify>Service methods correctly query/update the `applications` collection.</verify>
  <done>Backend support for applicant tracking is ready.</done>
</task>
...
<task type="auto">
  ...
  <action>
    ...
    Implement `KanbanBoard` with columns for each status.
