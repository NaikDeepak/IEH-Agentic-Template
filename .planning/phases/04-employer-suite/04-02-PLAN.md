...
<task type="auto">
  <name>Task 1: AI Screening Question Backend</name>
  <files>functions/index.js</files>
  <action>
    Add a new Cloud Function handler `generateQuestionsHandler`.
    Prompt Gemini 2.0 Flash to generate 3-5 screening questions based on the JD.
    Return JSON: `[{ "question": string, "hint": string }]`.
    Register route `/ai/generate-questions` (authenticated).
  </action>
  <verify>Test endpoint with curl/Postman using a sample JD.</verify>
  <done>Backend can generate structured screening questions from text.</done>
</task>
...
<task type="auto">
  ...
  <action>
    ...
    When generating Questions: call `/ai/generate-questions`, display questions in a list where employer can edit/remove them.
    Style using the IEH Brutalist design language.
  </action>
  <verify>Clicking 'AI Generate' populates fields; job saves with questions and company_id.</verify>
  <done>Employer has an AI-assisted workflow for high-quality job postings.</done>
</task>
