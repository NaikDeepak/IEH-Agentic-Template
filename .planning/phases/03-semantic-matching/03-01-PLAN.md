...
<task type="auto">
  ...
  <action>
    ...
    5. Create `scripts/verify_search.js` that checks for the existence of `searchCandidatesHandler` and `runVectorSearch` in `functions/index.js`.
  </action>
  <verify>node scripts/verify_search.js</verify>
